...
> 어디선가 유입이 자꾸 들어오고 있는 것 같은데.. 혹시 시간 되신다면 어느 쪽 링크 통해서 들어오셨는지 기재 부탁드리겠습니다. [어떻게 엄랭을 발견하셨나요..!](https://github.com/rycont/umjunsik-lang/issues/1)
...
영문 표기를 해야 할 때는 'umjunsik-lang'이라고 표기해주세요.
...
* [노드JS (v2)](https://github.com/rycont/umjunsik-lang/tree/master/umjunsik-lang-node): Deno 구현체의 NodeJS 포팅 버전입니다.
...
엄랭은 '엄', '준', '식', '동탄' 네 개의 키워드와 '!', '.', ' ', '~', 'ㅋ' 다섯 개의 기호로 코드가 이루어집니다.
...
변수는 인덱싱(정수)을 통해 접근하고 대입할 수 있습니다. 지정하지 않았을 경우 모든 변수의 기본값은 0입니다.

### 대입(엄)

연음의 개수 번째 변수에 뒤에 오는 수를 대입합니다.
...
`동탄{정수}?{실행할 명령}`으로 작성합니다. 정수가 0이라면 `실행할 명령`이 실행되며, 그렇지 않다면 다음 줄로 넘어갑니다.

### 준

`준` 뒤에 오는 정수 번째 줄로 이동합니다. `준.. => 2번째 줄(글자)로 이동`. 원라인코드의 경우에는 `~`로 분리된 코드 단위로 카운트하여 이동합니다.
...
[위키를 참고해주세요](https://github.com/rycont/umjunsik-lang/wiki)
